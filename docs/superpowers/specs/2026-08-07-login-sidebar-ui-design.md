...
- **`components/ui/sidebar.tsx`** — ported from the 719-line web source, with these deliberate omissions (approved simplifications, not silent deviations):
  ...
  - `SidebarSeparator` and its `Separator` import are dropped — the web source's `Separator` wraps `@radix-ui/react-separator`, which is not a `desktop-node` dependency, and nothing in this app's nav renders a separator (confirmed: `app-sidebar.tsx`'s 4 `NavMain` groups don't use one). Adding a new dependency for an unused component isn't justified.
  - Everything else (`SidebarProvider`, `Sidebar`, `SidebarTrigger`, `SidebarRail`, `SidebarInset`, `SidebarHeader`, `SidebarFooter`, `SidebarContent`, `SidebarGroup*`, `SidebarMenu*` except the omissions above) ports verbatim, including the cookie-based collapsed-state persistence (`sidebar_state` cookie, 7-day max-age) and the `Cmd/Ctrl+B` toggle shortcut.
